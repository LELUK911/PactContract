 ...
 */
function setEcosistemAddress(address _contract, bool _state) external onlyOwner {
 ...
 */
function showEcosistemAddressState(address _contract) external view returns (bool) {
    ...
    nonReentrant
{
    ...
    require(
        TimeManagment.checkDatalistAndExpired(_couponMaturity, _expiredBond),
...
function _subtractionPrizePoin(
    ...
    // Loan in [5e19, 1e20)
    if (
        bond[_id].sizeLoan >= 50e18 && 
        bond[_id].sizeLoan < 100e18
    ) {
    ...
    // Loan in [1e21, 5e21)
    if (
        bond[_id].sizeLoan >= 1000e18 && 
        bond[_id].sizeLoan < 5000e18
    ) {
...
function _claimScorePoint(uint _id, address _issuer) internal {
    ...
    uint totalPoints = prizeScore[_id][_issuer] + prizeScoreAlreadyClaim[_id][_issuer];
...
function _liquidationFee(
    ...
    // Medium-high score => 1.5%? [*But the code uses '1']
    if (
        conditionOfFee[_iusser].score > 700000 &&
        conditionOfFee[_iusser].score <= 1000000
    ) {
    ...
    // Medium-low score => 3%? [*But the code uses '2']
    if (
        conditionOfFee[_iusser].score > 500000 &&
        conditionOfFee[_iusser].score <= 700000
    ) {
    ...
    // Low score => 5%? [*But the code uses '4']
    if (conditionOfFee[_iusser].score <= 500000) {
        return _updateBalanceContractForEmissionNewBond(
            _tokenAddress,
            _amountCollateral,
...
function _couponFee(
    ...
    return _upDateBalanceUserFees(_tokenAddress, _amount, 50);
}
...
function _upDateBalanceUserFees(
    ...
    // Currently returns a hard-coded 0.5% instead of `(_amount * _fee) / 1000`
